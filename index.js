...
client.on("message", async message => {
  ...
  if (message.content.match(/(?<=\[\[)(.*)(?=\]\])/g)) {
    //wow i actually managed to make a regex
    ...
    request.get(
      `https://cydia.s0n1c.org/cydia/?url=${link}`,
      (err, resp, body) => {
        //TODO: test this
        ...
        const embed = new Discord.MessageEmbed()
          ...
          .addField(
            "Add to cydia",
            `[Click here.](https://cydia.saurik.com/api/share#?source=${link})`,
            true
          )
          .addBlankField()
          .addField(
            "Version",
            `${body.info.Version}, ${body.info.Codename}`,
            true
          )
          ...
          .addField(
            `Sections (${body.section_count})`,
            `${Object.keys(body.sections).join("\n")}`,
            true
          );
        message.channel.send(embed);
      }
